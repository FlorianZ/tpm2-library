...
fn main() {
    ...
    for (i, line) in RESPONSE_DATA.lines().enumerate() {
        ...
        let success = run_test(&test_name, || {
            let (cc_hex_str, hex_str) = trimmed.split_once(' ').expect("invalid format");
            ...
            let built_len = match parse_result {
                Ok((rc, Some((body, sessions)))) => {
                    ...
                    let resp_struct = match body {
                        ...
                        TpmResponseBody::Hmac(r) => {
                            tpm_build_response(&r, &sessions, rc, &mut writer)
                        }
                        TpmResponseBody::Import(r) => {
                            tpm_build_response(&r, &sessions, rc, &mut writer)
                        }
                        TpmResponseBody::Load(r) => {
                            tpm_build_response(&r, &sessions, rc, &mut writer)
                        }
                        TpmResponseBody::Quote(r) => {
                            tpm_build_response(&r, &sessions, rc, &mut writer)
                        }
