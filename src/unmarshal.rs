...
use tpm2_protocol::{
    ...
    data::{
        ...
        TpmEccCurve, TpmHt, TpmRh, TpmSt, TpmaLocality, TpmaNv, TpmaObject, TpmlPcrSelection,
        TpmsContext, TpmsCreationData, TpmsEccParms, TpmsEccPoint, TpmsKeyedhashParms,
        TpmsNvPublic, TpmsPcrSelect, TpmsPcrSelection, TpmsRsaParms, TpmsSchemeHash, TpmsSchemeXor,
...
impl TpmUnmarshal for Tpm2bPublic {
    fn unmarshal(buffer: &[u8]) -> TpmResult<(Self, &[u8])> {
        let (inner, remainder) = unmarshal_tpm2b_struct::<TpmtPublic>(buffer)?;
...
impl TpmUnmarshal for Tpm2bCreationData {
    fn unmarshal(buffer: &[u8]) -> TpmResult<(Self, &[u8])> {
        let (inner, remainder) = unmarshal_tpm2b_struct::<TpmsCreationData>(buffer)?;
        Ok((Self { inner }, remainder))
    }
}
...
impl TpmUnmarshal for Tpm2bNvPublic {
    fn unmarshal(buffer: &[u8]) -> TpmResult<(Self, &[u8])> {
        let (inner, remainder) = unmarshal_tpm2b_struct::<TpmsNvPublic>(buffer)?;
        Ok((Self { inner }, remainder))
    }
}

fn unmarshal_tpm2b_struct<T: TpmUnmarshal>(buffer: &[u8]) -> TpmResult<(T, &[u8])> {
...
impl TpmUnmarshal for TpmsCreationData {
    fn unmarshal(buffer: &[u8]) -> TpmResult<(Self, &[u8])> {
        let (pcr_select, buffer) = TpmlPcrSelection::unmarshal(buffer)?;
        let (pcr_digest, buffer) = Tpm2bDigest::unmarshal(buffer)?;
        let (locality, buffer) = TpmaLocality::unmarshal(buffer)?;
        let (parent_name_alg, buffer) = TpmAlgId::unmarshal(buffer)?;
        let (parent_name, buffer) = Tpm2bName::unmarshal(buffer)?;
        let (parent_qualified_name, buffer) = Tpm2bName::unmarshal(buffer)?;
        let (outside_info, buffer) = TpmBuffer::unmarshal(buffer)?;
...
impl TpmUnmarshal for TpmsNvPublic {
    fn unmarshal(buffer: &[u8]) -> TpmResult<(Self, &[u8])> {
        let (nv_index, buffer) = TpmUint32::unmarshal(buffer)?;
        let (name_alg, buffer) = TpmAlgId::unmarshal(buffer)?;
        let (attributes, buffer) = TpmaNv::unmarshal(buffer)?;
        let (auth_policy, buffer) = TpmBuffer::unmarshal(buffer)?;
        let (data_size, buffer) = TpmUint16::unmarshal(buffer)?;
...
impl TpmUnmarshal for TpmtTkCreation {
    fn unmarshal(buffer: &[u8]) -> TpmResult<(Self, &[u8])> {
        let (tag, buffer) = TpmSt::unmarshal(buffer)?;
        let (hierarchy, buffer) = TpmRh::unmarshal(buffer)?;
        let (digest, buffer) = TpmBuffer::unmarshal(buffer)?;
        Ok((
            Self {
                tag,
                hierarchy,
...
impl TpmUnmarshal for VtpmPolicySecretCommand {
    fn unmarshal(buffer: &[u8]) -> TpmResult<(Self, &[u8])> {
        let (object_handle_hint, buffer) = TpmUint32::unmarshal(buffer)?;
        let (object_name, buffer) = TpmBuffer::unmarshal(buffer)?;
        let (policy_ref, buffer) = TpmBuffer::unmarshal(buffer)?;
        Ok((
            Self {
                object_handle_hint,
                object_name,
